...
export class ChatRoomsService {
  ...
  async createRoom(currentUserId: number, dto: CreateRoomDto) {
    ...
    const users = await Promise.all(
      uniqueUsernames.map((u) => this.usersService.findOne(u)),
    );
    const existingUsers = users.filter((u): u is NonNullable<typeof u> => !!u);
    ...
    const participants = [currentUser, ...existingUsers].filter(
