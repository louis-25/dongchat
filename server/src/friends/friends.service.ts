...
export class FriendsService {
    ...
    async respondRequest(requestId: number, userId: number, action: 'accept' | 'block') {
        ...
        request.status = action === 'accept' ? FriendStatus.ACCEPTED : FriendStatus.BLOCKED;
    ...
    async listPending(userId: number) {
        return this.friendRepository.find({
            where: [
                { receiver: { id: userId }, status: FriendStatus.PENDING },
            ],
        });
    }
}
