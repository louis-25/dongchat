...
export class AuthController {
    ...
    @ApiErrorResponse(401, '비밀번호가 일치하지 않습니다.', 'AUTH_INVALID_PASSWORD')
     ...
     */
    @Post('refresh')
    @ApiOperation({ summary: '토큰 갱신', description: 'Refresh 토큰으로 새로운 Access 토큰을 발급합니다.' })
    @ApiBody({ type: RefreshTokenDto })
    @ApiResponse({ status: 200, description: '토큰 갱신 성공', type: AuthResponseDto })
    @ApiErrorResponse(401, '토큰이 만료되었거나 유효하지 않습니다.', 'AUTH_UNAUTHORIZED')
